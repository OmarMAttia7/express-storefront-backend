...
export default async function validateAddProduct(
  ...
  try {
    ...
    if(!verifyObjectProperties(body, ["product_name", "category", "price", "currency"])) {
      return res.status(400).json({error: "Error 400: incorrect syntax, modify request body."});
    }

    // Check if price is a valid number
    if(Number.isNaN( Number(body.price) )) {
      return res.status(400).json({error: "Error 400: incorrect syntax, price must be a valid number."});
    }
    ...
    if((await currencies.show(body.currency)) === undefined) {
      return res.status(404).json({error: "Error 404: this currency doesn't exist."});
    }
